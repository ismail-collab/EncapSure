...
<div class="container">
    <form id="DevisForm" action="DevisProc" method="POST">
        <?php echo csrf_field(); ?>
            ...
            <?php $__currentLoopData = $INFOS; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $inf): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="swiper-slide">
                    <div class="card three">
                        <div class="top">
                            <div class="title"><?php echo e($inf['pack_name']); ?></div>
                            <div class="price-sec">
                                <span class="price"><u><?php echo e($inf['price']); ?>DT</u></span>
                            </div>
                        </div>
                        <div class="details">
                            <?php 
                        ...
                        <input type="radio" name="pack" value="<?php echo e($inf['pack_id']); ?>" id="pack<?php echo e($inf['pack_id']); ?>" required>
                        <input type="hidden" name="money" value="<?php echo e($inf['price']); ?>" id="<?php echo e($inf['pack_id']); ?>" >
                        <label>Choisir</label>
                    </div>
                </div>
...
document.getElementById("next").addEventListener("click", function() {
    // Supposons que currentForm est la variable qui indique le formulaire actuel
    if(currentForm == 'F3') {
        document.getElementById("loading-spinner").style.display = "block";

        setTimeout(function() {
            document.getElementById("loading-spinner").style.display = "none";
        }, 5000); // Affiche le spinner pendant 5 secondes pour le formulaire F3
    }
    else if(currentForm == 'F4') {
        document.getElementById("loading-spinner").style.display = "block";

        setTimeout(function() {
            document.getElementById("loading-spinner").style.display = "none";
        }, 3000); // Affiche le spinner pendant 3 secondes pour le formulaire F4
    }
});
...
var client= new HttpClient();
  ...
  document.getElementById("immat_type").addEventListener("change", function() {
    ...
    const pack = document.getElementsByName("pack");
    const swip = document.getElementById('swip');
...
if (prog==3) {
  ...
  if (document.querySelector('input[type=radio]:checked')) { 
    ...
    // mock api systeme d'information assurance

    if (immat.value.trim() == '1234' || immat.value.trim() == '2345' || immat.value.trim() == '3456' ) {
    // Modifier le contenu HTML de la section
    document.querySelector('.modal-box').innerHTML = `
    <i class="fas fa-ban"></i>
    <h3>L'immatriculation fournie n'existe pas dans notre système d'assurance, il n'est donc pas possible de convertir la simulation en devis.</h3>
    <h4 id="amount">Informations véhicule incorrectes</h4>
    <div class="buttons">
        <button type="reset" id="convert_btn" class="close-btn" style="display: none;">Oui, convertir</button>
        <button id="down_btn" classe="btn-pop" onclick="Cancelation();" class="down_btn">Annuler</button>
    </div>
    `;
...
money= document.getElementById(document.querySelector('input[name="pack"]:checked').value).value;
 // amount.innerHTML = "Le montant total de devis est : "+(parseInt(price_new.value.trim()/10000)+money)+" TND";
...
swip.style.display="none";
  ...
  // mock api blacklist  
    ...
    <h3> <br> Le CIN que vous avez fourni est malheureusement inscrit sur notre liste noire, ce qui nous empêche de traiter votre demande d'assurance.</h3>
    <h4 id="amount">L'opération ne peut pas être réalisée</h4>
    <div class="buttons">
        <button id="down_btn" class="btn-pop" onclick="Cancelation();" class="down_btn">Fermer</button>
    </div>
    `;
    section.classList.add("active");
    swip.style.display="none";
...
</html><?php /**PATH C:\xampp\htdocs\InsucranceFrontEnd\InsucranceFrontEnd\project\resources\views/DevisForms.blade.php ENDPATH**/ ?>