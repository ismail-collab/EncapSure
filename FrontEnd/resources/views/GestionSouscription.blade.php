        ...
        <h1 class="text-center mb-5 interactive client-text"> <i class="fas fa-user-shield"></i> Consulter souscriptions clients</h1>
            ...
            <table>
                ...
                @foreach ($sub as $sb)
                    <tr>
                        <td>{{$sb->sub_id}}</td>
                        <td>{{$sb->client_firstname}} {{$sb->client_lastname}}</td>
                        <td>{{$sb->email}}</td>
                        <td>{{$sb->effect_date}}</td>
                        <td>{{$sb->money}} TND </td>
                        <div id='pops-content{{$sb->sub_id}}' class='pops-content visite-pdf'>
                        <iframe height='700' width='800' src='data:application/pdf;base64,{{$sb->contract}}' ></iframe>
                        <div class='close-button-wrapper'>    
                            <button id='confirm' onClick='CloseIt({{$sb->sub_id}});'>Close</button>
                        </div>
                        </div>   
<td>
<button class="btn-view" onClick='Show({{$sb->sub_id}});'><i class="fas fa-eye"></i></button>     
