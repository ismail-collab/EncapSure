...
<div class="navbar">
  ...
  <div class="nav-items">
  <a href="HomeAdmin" class="nav-link"><i class="fas fa-user"></i> {{ session()->get('name') }}</a>
...
<div class="container">
    ...
    @php for ($i=1;$i<count($inf)-2;$i++) {
        if ($inf['gr'.$i]!=0) { echo '<td><i class="fas fa-check"></i></td>'; } else { echo '<td>-</td>'; } 
            ...
            <td>
                <button class="btn-edit" id="Mod_{{$inf['pack_id']}}" onClick="editPack('{{$inf['pack_id']}}','{{$inf['pack_name']}}','{{$inf['price']}}',@php   for ($i=1;$i<count($inf)-2;$i++) { if ($i==count($inf)-3) { echo $inf['gr'.$i]; } else { echo  $inf['gr'.$i].','; } }  @endphp)"><i class="fas fa-edit"></i></button>
                <button class="btn-delete" id="Del_{{$inf['pack_id']}}" onClick="Delete('{{$inf['pack_id']}}')"><i class="fas fa-trash-alt"></i></button>
            </td>
        </tr>
    @endforeach
    </tbody>
