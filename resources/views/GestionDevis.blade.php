...
<div class="content">
    ...
    <div class="container">
        ...
        <table id="dataTable">
            ...
            @php  $dev=$DEVIS['DEVIS']; for ($i=0;$i<count($dev);$i++) { $thisD = $dev[$i]->devis_id; @endphp
                <tr>
                    <td>{{$dev[$i]->devis_id}}</td>
                    <td>{{$dev[$i]->client_firstname}}  {{$dev[$i]->client_lastname}}</td>
                    <td>{{$dev[$i]->email}}</td>
                    <td>{{$dev[$i]->phone}}</td>
                    <td><a href="DownDFiles/{{$thisD}}">Download</a></td>
                    <div id='pops-content{{$dev[$i]->devis_id}}' class='pops-content visite-pdf'>
                        <iframe height='700' width='800' src='data:application/pdf;base64,{{$dev[$i]->devis_doc}}' ></iframe>
                        <div class='close-button-wrapper'>    
                            <button id='confirm' onClick='CloseIt({{$dev[$i]->devis_id}});'>Close</button>
                        </div>
                    </div>     
<td>
    <button class="btn-view" onClick="Show({{$dev[$i]->devis_id}})"><i class="fas fa-eye"></i></button>             
    <!-- <button class="btn-edit" onclick=""><i class="fas fa-edit"></i></button>-->
    <button class="btn-delete" onClick="Delete({{$dev[$i]->devis_id}})"><i class="fas fa-trash-alt"></i></button> 
</td>
                </tr>
            @php } @endphp
            </tbody>
