...
<tr>
    ...
    <tr>
    <td><input type="checkbox" name="recps[{{$i+=1}}]" value="{{$prod->receipt}}-{{$prod->contract_num}}-{{$prod->left_amount}}"></td>
    <td>{{$prod->product}}</td>
...
<div class="navbar">
  ...
  <div class="nav-items">
  <a href="Home" class="nav-link"><i class="fas fa-user"></i> {{ session()->get('name') }}</a>
...
<tr>
    @php $i=0 @endphp
    @foreach ($PRODUCT as $prod)
    @php if ($prod->left_amount!=0) { @endphp
    <tr>
    <td><input type="checkbox" name="recps[{{$i+=1}}]" value="{{$prod->receipt}}-{{$prod->contract_num}}-{{$prod->left_amount}}"></td>
    <td>{{$prod->product}}</td>
    <td>{{$prod->contract_num}}</td>
