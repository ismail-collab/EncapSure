...
<!--
                    <td>{{$dl->client_lastname}}</td>
                    <td>{{$dl->client_firstname}}</td>
                    <td>{{$dl->client_email}}</td>
-->
                    <td>{{$dl->contract_num}}</td>
<!--
                    <td>{{$dl->receipt}}</td>
-->
                    <td>{{$dl->start_date}} Au {{$dl->end_date}}</td>
                    <td>{{$dl->left_amount}}</td>

                    @php if ($dl->state=="Waiting for documents upload" || $dl->state=="Not Paid" || $dl->state=="Waiting for payment" || $dl->state=="Waiting documents submission") { @endphp

                        <td> No document : {{$dl->state}} </td>
                        <td> No document : {{$dl->state}} </td>
                        <td>{{$dl->state}}</td>


                    @php } else if ($dl->state=="Completed") { @endphp


                        <td>
                        <a href="#" onClick="ShowPOPS({{$dl->contract_num}});" id="vig_view"><i class="fas fa-eye"></i> View</a>
<div id="pops{{$dl->contract_num}}" class="pops">
              
    <div class="pops-content vignette-pdf">
        <iframe height="800" width="800" src="data:application/pdf;base64,{{$dl->vignette}}" ></iframe>
        <div class="close-button-wrapper">
            <button  id="confirm" onClick="ClosePOPS({{$dl->contract_num}});">Close</button>
        </div>
    </div>      
</div>
                    </td>
                    <td>
                        <a href="#" onClick="ShowPOPS({{$dl->receipt}});" id="tv_view"><i class="fas fa-eye"></i> View</a>
<div id="pops{{$dl->receipt}}" class="pops">

<div class="pops-content visite-pdf">
    <iframe height="800" width="800" src="data:application/pdf;base64,{{$dl->tech_visit}}" ></iframe>
     <div class="close-button-wrapper">
        <button id="confirm" onClick="ClosePOPS({{$dl->receipt}});">Close</button>
     </div>
</div>
            </div>
                    </td>
...
<div id="THEBTNS{{$dl->receipt}}">
    <button class="validate" onClick="ValidVig({{$dl->receipt}});" >Valider</button>
    <button class="reject" onClick="ShowOV('ov{{$dl->receipt}}');" >Rejeter</button>
</div>
                    </td>
                    <td>
                        <a href="#" onClick="ShowPOPS({{$dl->receipt}});" id="tv_view"><i class="fas fa-eye"></i> View</a>
...
<div class="pops-content visite-pdf">
    <iframe height="800" width="800" src="data:application/pdf;base64,{{$dl->tech_visit}}" ></iframe>
     <div class="close-button-wrapper">
        ...
        <div id="ov{{$dl->receipt}}" class="overlay">
            <div class="popup">
                <div>
                <h3>Motif de rejet</h3>
                <select name="motif" class="motif" id="reason{{$dl->receipt}}">
            ...
            <div class="bons">
                <button class="confirm" onClick="RejectVig({{$dl->receipt}});">Confirmer</button>
                <button class="cancel" onClick="CloseOV('ov{{$dl->receipt}}')">Annuler</button>
                </div>
        </div>


        <div id="ov{{$dl->receipt}}2" class="overlay">
            <div class="popup">
                <div>
                <h3>Motif de rejet</h3>
                <select name="motif" class="motif" id="reason{{$dl->receipt}}2">
                    <option value="">Sélectionner un motif</option>
                    <option value="documents_flous">Documents flous</option>
                    <option value="documents_non_valides">Documents non valides</option>
                    <option value="document_expire">Document expiré</option>
                ...
                <div class="bons">
                <button class="confirm" onClick="RejectTech({{$dl->receipt}});">Confirmer</button>
                <button class="cancel" onClick="CloseOV('ov{{$dl->receipt}}2')">Annuler</button>

            </div>
            </div>

        </div>
        <div id="THEBTNS{{$dl->receipt}}2">
                        <button class="validate" onClick="ValidTech({{$dl->receipt}});" >Valider</button>
                        <button class="reject" onClick="ShowOV('ov{{$dl->receipt}}2');"  >Rejeter</button>
                    </div>
                    </td>
                    <td>{{$dl->state}}</td>
